...
if [ -e secrets/interrealm_bridge_config_scicomp_cs.py ] ; then
    kubectl delete secret generic zulip-cs-sc-bridge $@ --ignore-not-found=true
    kubectl create secret generic zulip-cs-sc-bridge $@ --from-file=interrealm_bridge_config=secrets/interrealm_bridge_config_scicomp_cs.py
fi
